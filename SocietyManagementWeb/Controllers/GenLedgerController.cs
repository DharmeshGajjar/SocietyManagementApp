        ...
        public IActionResult ExportToExcelPDF(int gridMstId, string searchValue, int type,string toDate,string accountId,string fromDate,string departmentId)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        var departmentDetails = DbConnection.GetDepartmentDetailsById(Convert.ToInt32(departmentId), companyId);
                        var bytes = PDF(getReportDataModel, "General Ledger Report", departmentDetails.DepName, departmentId);
                        return File(
                              bytes,
                              "application/pdf",
                              "GenLedger.pdf");
                    ...
                    {
                        var bytes = PDF(getReportDataModel, "General Ledger Report", companyDetails.CmpName, "");
                        return File(
                              bytes,
                              "application/pdf",
                              "GenLedger.pdf");
